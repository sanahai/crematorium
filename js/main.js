(function() {
  ...
  function filterAndRender() {
    ...
    const filtered = allData.filter(item => {
      ...
      const matchSearch = !q || (item.name && item.name.toLowerCase().includes(q)) || (item.address && item.address.toLowerCase().includes(q));
  ...
  function renderCards(list) {
    cardGrid.innerHTML = '';
    if (list.length === 0) { showState('empty'); resultCount.innerHTML = '검색 결과 <strong>0</strong>건'; return; }
    showState('list');
    ...
    list.forEach(item => {
      ...
      card.innerHTML = `
        ...
        <div class="card-footer"><button class="card-detail-btn" onclick="goDetail('${item.id}')">상세 정보 보기</button></div>`;
      cardGrid.appendChild(card);
    });
  }

  window.goDetail = id => { location.href = `detail.html?id=${encodeURIComponent(id)}`; };
  function showState(state) {
    loadingBox.classList.toggle('hidden', state !== 'loading');
    emptyBox.classList.toggle('hidden', state !== 'empty');
    cardGrid.classList.toggle('hidden', state === 'loading' || state === 'empty');
  }
